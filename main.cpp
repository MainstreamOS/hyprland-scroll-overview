...
typedef void (*origScheduleFrameForMonitor)(void*, Aquamarine::IOutput::scheduleFrameReason);
...
static void hkScheduleFrameForMonitor(void* thisptr, Aquamarine::IOutput::scheduleFrameReason reason) {
    const auto PMONITOR = sc<Monitor::CMonitor*>(thisptr);

    if (g_pScrollOverview && PMONITOR && g_pScrollOverview->pMonitor == PMONITOR->m_self) {
    ...
    rc<origScheduleFrameForMonitor>(g_pScrollScheduleFrameHook->m_original)(thisptr, reason);
}
